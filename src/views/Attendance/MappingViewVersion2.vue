...
const confirmAttendance = async () => {
  for (let i = 0; i < identifications.value.length; i++) {
    // Access the array inside `imageUrls` ref by using `.value[i]`
    const resizedImageBase64 = await resizeAndConvertToBase64(
      imageUrls.value[i], // Corrected access to the ref
      800,
      600
    ...
    const identifiedUser = identifications.value[i].name === "Unknown"
      ? null
    ...
    const attendanceData = {
      ...
      user: identifiedUser ? identifiedUser : undefined,
      attendanceImage: "",
    };

    console.log("Submitting:", attendanceData);
    try {
      await attendaceStore.createAttendance(attendanceData, imageFile);
          ...
          >
            <v-card outlined>
              <v-card-title class="white--text"
                >{{ name.studentId }} | {{ name.name }}</v-card-title
              >
              <v-img
